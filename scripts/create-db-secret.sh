...
cf8 2>/dev/null >/dev/null service $NAME || cf8 create-service hana hdi-shared $NAME
while true; do
    STATUS="$(cf8 2>/dev/null service $NAME | grep status: | head -n 1)"
    echo $STATUS
...
cf8 create-service-key $NAME $NAME-key

node "$(dirname "$0")/format-kyma-secret.js" -- "$(echo "$SECRET_HEADER")" "$(cf8 service-key $NAME $NAME-key)" | kubectl apply -f -
echo
